class Player(object):
    ...
    def expected_score_against(self, opponent):
        if self.rating == 0:
            return 0
    ...
    def expected_score_against_many(self, opponents):
        if self.rating == 0:
            return 0

        opponents = [x for x in opponents if x.rating > 0]

        fake_opponent = Player("Fake", sum([x.rating for x in opponents]) / len(opponents))

        return self.expected_score_against(fake_opponent)
